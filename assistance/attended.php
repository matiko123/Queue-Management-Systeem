...
if(!isset($_SESSION['assistance'])){
    ...
    $id=$_SESSION['assistance'];
      ...
      $queryn = "SELECT * from assistance inner join service on service.id=assistance.service where assistance.id=$id";
      $resultn = mysqli_query($conn, $queryn);
      while($row= $resultn->fetch_assoc()) {
        $service_id=$row['service'];
      }
  //query selector for admin tokens
  $query = "SELECT assistance.*,service.* FROM assistance inner join service on assistance.service=service.id where assistance.id= $id";
  ...
  //query selector for admin tokens
  $query1 = "SELECT * from customer where service_name=$service_id and attendance=1" ;
